...
| shikodachi | 四股立 (四股立ち) | horse stance — wide, low, sumo-style (四股 = the sumo leg-stomp; standard karate stance name). Sword-Waza.md's `Shikodachi` entry is a Chiba partner striking drill done in this stance. The earlier `仕込太刀` reading (仕込 shikomu "to prepare" + 太刀) was a guess and is **retracted**. See variances. |
...
- **shikodachi — 四股立 vs 四股立ち**: `しこだち` is the sumo/karate horse stance; jisho/karate texts write it 四股立ち (with okurigana), this repo uses the bare 四股立 (Sword-Waza.md heading, Glossary term). Sword-Waza.md's `Shikodachi` entry is a Chiba partner striking drill *done in* that stance — the name is the stance. The earlier kanji `仕込太刀` (a "prepare the sword" reading) was a guess from a video and is retracted.
...
- **User** — しこだち is 四股立 (四股立ち), the horse stance; the Chiba drill in Sword-Waza.md is partner striking done in that stance (correcting an earlier `仕込太刀` kanji guess, now retracted).
